...
class Amp():
    ...
    def setVolume(self, vol):
        # VOLUME 0-36
        self.volume = vol
        RPiMIB.sendI2C(self.address, self.volume)
...
class FG():
    ...
    def sendData(self, input):
        ...
        data1 = input >> 8
        data2 = input & 0xFF
        RPiMIB.sendSPI(self.chipSelect, data1, data2) # command to write the next data to Function Generator 1
    ...
    def sendFreq(self, freq):
        ...
        #DB15 and DB14 are set to 0 and 1
        self.MSB |= 0x4000
        #DB15, DB14,DB13 = 110 DB12 = x    #respectively, which is the address for Phase Register 0.
        #The remaining 12 bits are the data bits and are all 0s in this case
        self.phase |= 0xC000
        self.sendData(0x2100)
        print(freq)
        self.sendData(self.MSB)
        self.sendData(self.LSB)
        self.sendData(self.phase)
        self.sendData(0x2000)
